...
function DispenseHistoryRow({ item, user }: { item: DispenseHistory, user: UserProfile | undefined }) {
        ...
        <Collapsible asChild open={isOpen} onOpenChange={setIsOpen}>
            <TableBody>
                <TableRow>
                    ...
                    <TableCell className="font-medium text-destructive hidden md:table-cell">-{item.pointsUsed}</TableCell>
                    <TableCell className="hidden md:table-cell">{item.dispenserIndex}</TableCell>
                        ...
                        <TableCell colSpan={4} className="pt-0">
                           <div className="p-4 space-y-4 bg-muted/50 rounded-md">
                                <div className="flex justify-between items-center">
                                    <span className="font-medium text-muted-foreground">Points Used</span>
                                    <span className="font-medium text-destructive">-{item.pointsUsed}</span>
                                </div>
                                <div className="flex justify-between items-center">
                                    <span className="font-medium text-muted-foreground">Dispenser</span>
                                    <span>{item.dispenserIndex}</span>
                                </div>
                                <div className="flex justify-between items-center">
...
export function DispenseHistoryTable() {
  const firestore = useFirestore();

  const historyQuery = useMemo(() => {
    if (!firestore) return null;
    return query(collection(firestore, 'dispense_history'), orderBy('timestamp', 'desc'));
  }, [firestore]);
  ...
  const { data: history, loading: historyLoading } = useCollection<DispenseHistory>(historyQuery);
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>A log of all rewards dispensed to users.</CardDescription>
      </CardHeader>
        ...
        <Table>
          <TableHeader>
            <TableRow>
              ...
              <TableHead className="hidden md:table-cell">Points Used</TableHead>
              <TableHead className="hidden md:table-cell">Dispenser</TableHead>
            ...
            {loading ? (
              ...
              history.map((item) => {
                ...
                return (
                 <DispenseHistoryRow key={item.id} item={item} user={user} />
                );
              })
            ) : (
                <TableBody>
